...
const {asyncHandler, handleValidationErrors, check, validationResult, bcrypt, csrfProtection} = require('../utils');
const db = require('../db/models');
const { Post, User, Topic, Comment } = db;

router.get(/\/\d+/, asyncHandler(async (req, res) => {
    const postId = req.url.slice(1)
    let post = await Post.findOne({where: {id: postId},include: [{model: User}, {model: Topic}]});
    let comments = await Comment.findAll({where: {postId}, include: {model: User}});
